...
async function submitTask(page, opts) {
    ...
    // === 步骤 5: 上传参考文件（图片/视频/音频） ===
    const allFiles = [...images, ...videos, ...audios];
    log(5, `上传 ${images.length} 图片 + ${videos.length} 视频 + ${audios.length} 音频...`);
    ...
    log(5, `页面有 ${fileInputs.length} 个 file input`);
    if (fileInputs.length < 1) throw new Error('未找到 file input');

    // 判断上传模式
    const hasMultiInput = fileInputs.some(fi => fi.multiple && fi.accept.includes('video'));

    if (hasMultiInput) {
        // 模式 A：单个 multiple input 接受所有类型（如"全能参考"）
        const multiIdx = fileInputs.findIndex(fi => fi.multiple && fi.accept.includes('video'));
        log(5, `多文件上传模式 (input index: ${multiIdx})，共 ${allFiles.length} 个文件`);
        await page.locator('input[type="file"]').nth(multiIdx).setInputFiles(allFiles);
        log(5, `全部 ${allFiles.length} 个文件上传完成`);
        await sleep(3000);
    } else {
        // 模式 B：多个独立 input（如"首尾帧"每个 input 一张图片）
        for (let i = 0; i < images.length; i++) {
            const targetIdx = Math.min(i, fileInputs.length - 1);
            await page.locator('input[type="file"]').nth(targetIdx).setInputFiles(images[i]);
            log(5, `图片${i + 1} 上传完成 (index: ${targetIdx})`);
            await sleep(3000);
        }
        if (videos.length > 0) log(5, `WARNING: 当前模式不支持视频上传，跳过 ${videos.length} 个视频`);
        if (audios.length > 0) log(5, `WARNING: 当前模式不支持音频上传，跳过 ${audios.length} 个音频`);
    }
        ...
        for (const part of promptParts) {
            if (part.type === 'text') {
                ...
                const atResult = await page.evaluate((label) => {
                    ...
                    for (const item of popups) {
                        ...
                        if (rect.width > 0 && rect.height > 0) {
                            ...
                            if (item.textContent.trim() === label) {
                                item.click();
                                return { text: item.textContent.trim(), matched: 'exact' };
                            }
                        }
                    }
                    // 严格匹配失败，尝试包含匹配
                    for (const item of popups) {
                        const rect = item.getBoundingClientRect();
                        if (rect.width > 0 && rect.height > 0 && item.textContent.trim().includes(label)) {
                            item.click();
                            return { text: item.textContent.trim(), matched: 'includes' };
                        }
                    }
                    return { allOptions };
                }, part.label);
                if (atResult && atResult.matched) {
                    log(6, `  @引用成功: "${atResult.text}" (${atResult.matched})`);
                } else {
                    const options = atResult?.allOptions || [];
                    log(6, `  ERROR: @"${part.label}" 未找到! 弹窗选项: [${options.map(o => `"${o}"`).join(', ')}]`);
                    atSuccess = false;
                    await page.keyboard.press('Backspace');
                }
                await sleep(500);
