...
class SessionPool {
    constructor(roxyConfig) {
        ...
        this.sessionConfigs = new Map(); // dirId -> cfg（保留原始配置用于重连）
    }
    ...
    async init(sessionConfigs) {
        for (const cfg of sessionConfigs) {
            this.sessionConfigs.set(cfg.dirId, cfg);
    ...
    /**
     * 重连指定 session：关闭旧连接，重新初始化
     */
    async reconnect(dirId) {
        const cfg = this.sessionConfigs.get(dirId);
        if (!cfg) throw new Error(`未找到 session 配置: ${dirId}`);

        console.log(`[SessionPool] 重连 session: ${cfg.name} (${dirId})`);

        // 清理旧连接
        const old = this.sessions.get(dirId);
        if (old) {
            try { await old.browser.close(); } catch {}
            this.sessions.delete(dirId);
        }

        await this._initSession(cfg);
        console.log(`[SessionPool] ${cfg.name} 重连成功`);
    }
